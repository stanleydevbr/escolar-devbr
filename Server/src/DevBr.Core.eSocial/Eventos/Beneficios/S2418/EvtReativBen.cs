...
{
    public class EvtReativBen : IdeEvento<EvtReativBen>, IeSocialAggregate
    {
        public IdeEmpregador? IdeEmpregador { get; set; }
        public IdeBeneficio? IdeBeneficio { get; set; }
        public InfoReativ? InfoReativ { get; set; }
        public EvtReativBen(uint tpAmb, uint procEmi, uint verProc, string evento, string version) : base(tpAmb, procEmi, verProc, evento, version)
        {
        }
        ...
        protected override XElement? GetXmlElements(XNamespace ns)
        {
            ...
            XElement element = new XElement(ns + nameof(EvtReativBen));
            element.Add(IdeEmpregador?.GetXmlElements(ns));
            element.Add(IdeBeneficio?.GetXmlElements(ns));
            element.Add(InfoReativ?.GetXmlElements(ns));
            return element;
